...
export function SearchBar({ className }: SearchBarProps) {
  ...
  return (
    <div
      className={cn(
        "flex items-center h-[45px] w-full max-w-[400px] min-w-[300px] gap-2 rounded-md border border-border-default bg-white px-3",
        className
      )}
    >
      <Icon name="search" className="w-4 h-4 md:w-5 md:h-5 text-text-secondary" />
      ...
      <div className="relative flex-1 min-w-0 h-full overflow-hidden">
        <input
          ...
          className="peer absolute inset-0 w-full h-full bg-transparent outline-none text-[15px] font-medium placeholder:text-transparent"
        />
        {/* Animated placeholder overlay - hidden once the input has a value or focus */}
        <div
          className="pointer-events-none absolute inset-0 flex items-center text-text-muted text-[15px] font-medium peer-focus:opacity-0 peer-[:not(:placeholder-shown)]:opacity-0"
          aria-hidden="true"
        >
          ...
          <span className="relative flex-1 min-w-0 h-[1.2em] overflow-hidden">
            <span
              ...
              className={cn(
                ...
                transitioning ? "placeholder-exit" : "placeholder-enter"
              )}
            >
              {current}
            </span>
